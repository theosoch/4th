...
int EVALUATE_LEVEL_4(Programme prog, char* dirname, int included_word_start, int excluded_word_end, Pile eval_context_stack, Dictionnaire* p_eval_context_dict) {
    ...
    for(int i = included_word_start; i < excluded_word_end && eval_status == EVALUATION_SUCCESS; ++i) {
        // printf("\n");
        // printf("//X| %d ; %d\n", i, SommetPile(WORD_I_JUMP_STACK));

        while(!PileVide(WORD_I_JUMP_STACK) && i >= SommetPile(WORD_I_JUMP_STACK)) {
        ...
        if(i < excluded_word_end) {
            ...
            if(is_int)
                ...
                // word == "repeat" -> ERROR
                else if(strEquals(word, (char*) eval_words[24])) {
                    eval_status = EVALUATION_ERROR_INVALID_LOOP_SYNTAX_MISSING_KEYWORD_WHILE;
                }
                ...
                // word == "include"
                else if(strEquals(word, (char*) eval_words[36]))
                    EVAL_INCLUDE(prog, dirname, excluded_word_end, eval_words, eval_word_count, i, eval_context_stack, p_eval_context_dict, &eval_status);
            }
            else if((word_dict_def_index = RechercherDico(word, *p_eval_context_dict)) > 0) {
                int def_end_index = searchClosingWord(ProgWordArray(prog), LongueurProg(prog), word_dict_def_index-2, (char*) eval_words[33], (char*) eval_words[34], 1);

                eval_status = evaluate(prog, dirname, word_dict_def_index, def_end_index, eval_word_count, eval_context_stack, p_eval_context_dict, -1);
            }
            else {
